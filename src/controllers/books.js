  ...
  Book.findById(book_id)
    .then((book) => {
      if (book.available === false) {
        return res
          .status(200)
          .send(`Книга ${book.title}, ${book.author} отсутствует`);
      } else {
        Book.findByIdAndUpdate(book_id, { available: false }).then(() => {
          return User.findByIdAndUpdate(user_id, {
            ...
            res
              .status(200)
              .send(`Вы взяли книгу ${book.title}, ${book.author}`);
          });
        });
      }
    })
  ...
  Book.findById(book_id)
    .then((book) => {
      if (book.available === true) {
        return res
          .status(200)
          .send(`Книга ${book.title}, ${book.author} уже сдана`);
      } else {
        return Book.findByIdAndUpdate(book_id, { available: true }).then(() => {
          User.findByIdAndUpdate(user_id, {
            ...
            res
              .status(200)
              .send(`Вы вернули книгу ${book.title}, ${book.author}`);
          });
        });
      }
    })
